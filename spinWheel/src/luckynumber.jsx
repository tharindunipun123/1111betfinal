...
const socket = io('http://145.223.21.62:3015');
...
const LuckyNumber = () => {
  ...
  const [timeRemainings, setTimeRemaining] = useState(30);
  ...
  const fetchWinningPercentages = () => {
    fetch('http://145.223.21.62:3015/winning-percentages')
  ...
  const fetchWallet = () => {
    fetch(`http://145.223.21.62:3015/wallet?user_id=${userId}`)
      .then(response => response.json())
  ...
  const checkBetResult = (roundNumber) => {
    ...
    fetch(`http://145.223.21.62:3015/check-bet-result/${userId}/${roundNumber}`)
      .then(response => response.json())
      .then(data => {
        if (data.message === 'win') {
  ...
  const fetchLastWinningNumbers = () => {
    fetch('http://145.223.21.62:3015/last-10-winning-numbers')
      .then(response => response.json())
  ...
  const handleBet = (multiplier) => {
    ...
    swal({
      ...
      if (willBet) {
        fetch('http://145.223.21.62:3015/bet', {
          method: 'POST',
          headers: {
    ...
    <div className="game-container">
      ...
      <main className="game-content">
        <section className="left-section">
          ...
          <div className="timer">
            <h2>Time Remaining: {timeRemainings} seconds</h2>
          </div>
