...
const socket = io('http://145.223.21.62:3021');
...
const DiceBettingGame = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const fetchWallet = () => {
        fetch(`http://145.223.21.62:3021/wallet?user_id=${userId}`)
    ...
    const handleNumberBetChange = (number, value) => {
      ...
      if (rangeBet.type === 'lower' && number > 9) {
        Swal.fire({
          title: 'Invalid Bet',
          text: 'You can only bet on LOWER when lower range is selected',
          icon: 'error'
        });
        return;
      }
      if (rangeBet.type === 'higher' && number <= 9) {
        Swal.fire({
          title: 'Invalid Bet',
          text: 'You can only bet on HIGHER when higher range is selected',
          icon: 'error'
        });
        return;
      }
    ...
    const placeBet = async () => {
      ...
      try {
        for (const bet of activeBets) {
          const response = await fetch('http://145.223.21.62:3021/bet', {
            method: 'POST',
            headers: {
    ...
    const checkResults = async () => {
      try {
        const response = await fetch(`http://145.223.21.62:3021/check-bet-result/${userId}/${currentRoundNumber}`);
        const data = await response.json();
  ...
  return (
    ...
    <div>
      ...
      <div>
        <h3 style={{ marginBottom: '15px' }}>Bet for Range</h3>
        <div style={styles.numberBettingGrid}>
        {[3, 18].map(value => (
  <div key={value}>
    <label style={styles.label}>Range {value === 3 ? 'Low' : 'High'}</label>
    <input
      type="number"
      style={styles.input}
      placeholder={`Set ${value === 3 ? 'Low' : 'High'} Range`}
      value={numberBets[value]}
      onChange={(e) => handleNumberBetChange(value, e.target.value)}
      disabled={isBettingLocked}
    />
  </div>
))}
