...
function getTotalBets($conn) {
    $sql = "SELECT COUNT(*) as total FROM lucky_betting_results";
...
function getBetPercentage($conn) {
    $sql = "SELECT multiplier, COUNT(*) as count FROM lucky_betting_results GROUP BY multiplier";
    $result = $conn->query($sql);
    $betPercentage = array();
    $total = 0;
...
function getProfitLoss($conn, $round = null) {
    ...
    $sql = "SELECT 
            ...
            FROM lucky_betting_results br
            JOIN lucky_rounds r ON br.round = r.round_number
            $whereClause";
...
function getRoundResults($conn) {
    $sql = "SELECT round_number, winning_multiplier FROM lucky_rounds ORDER BY id DESC LIMIT 10";
    $result = $conn->query($sql);
    $roundResults = array();
    while ($row = $result->fetch_assoc()) {
        $roundResults[] = $row;
...
function getBetPercentageForRound($conn, $round) {
    $sql = "SELECT multiplier, COUNT(*) as count FROM lucky_betting_results WHERE round = ? GROUP BY multiplier";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $round);
    $stmt->execute();
    $result = $stmt->get_result();
