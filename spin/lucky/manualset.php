...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // INSERT action
    if ($action == 'insert') {
        $stmt = $conn->prepare("INSERT INTO lucky_manual_set (round_number) VALUES (?)");
        if (!$stmt) {
    ...
    elseif ($action == 'update') {
        $id = $_POST['id'];
        $stmt = $conn->prepare("UPDATE lucky_manual_set SET round_number = ? WHERE id = ?");
        if (!$stmt) {
            die("Prepare failed: (" . $conn->errno . ") " . $conn->error);
        }
    ...
    elseif ($action == 'delete') {
        $id = $_POST['id'];
        $stmt = $conn->prepare("DELETE FROM lucky_manual_set WHERE id = ?");
        if (!$stmt) {
            die("Prepare failed: (" . $conn->errno . ") " . $conn->error);
        }
...
$result = $conn->query("SELECT * FROM lucky_manual_set ORDER BY id DESC LIMIT 100");
?>
