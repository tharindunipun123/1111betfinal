...
async function endRound() {
  try {
    ...
    // Determine the final multiplier based on the winning multiplier
    if (winningMultiplier === 3) {
      // Random selection from [3, 6, 9]
      const lowNumbers = [3, 6, 9];
      finalMultiplier = lowNumbers[Math.floor(Math.random() * lowNumbers.length)];
    } else if (winningMultiplier === 18) {
      // Random selection from [12, 15, 18]
      const highNumbers = [12, 15, 18];
      finalMultiplier = highNumbers[Math.floor(Math.random() * highNumbers.length)];
    }

    console.log(`Original winning multiplier: ${winningMultiplier}`);
    console.log(`Final multiplier for round ${currentRoundNumber}: ${finalMultiplier}`);

    // Update the current round with the final multiplier
    await db.query('UPDATE dice_rounds SET winning_multiplier = ? WHERE round_number = ?', [finalMultiplier, currentRoundNumber]);

    // Emit to frontend that the wheel should start spinning
    io.emit('spinWheel', { winningMultiplier: finalMultiplier, roundNumber: currentRoundNumber });
    ...
    setTimeout(async () => {
      ...
      for (const bet of bets) {
        // Calculate winnings based on whether the user bet on the original winning number (3 or 18)
      ...
      io.emit('roundResult', { roundId: currentRoundId, winningMultiplier: finalMultiplier, results });
...
app.get('/check-bet-result/:userId/:roundNumber', async (req, res) => {
  ...
  try {
    ...
    if (winningBet) {
      const winAmount = winningBet.amount * 1.8;
      return res.json({ 
        message: 'win', 
        multiplier: winingName,
